...
export function deleteReview (req,res) {
    ...
    if (req.user == admin) {
        Review.deleteOne({email : email}).then (()=>{
            res.json({message : "Review deleted succssfully!"});
        }).catch (()=>{
...
export function approveReviews (req,res) {
    ...
    if (req.user.role == "admin") {
        Review.updateOne(
        {
            email : email,       //it should be equal, not means equal and is it equal
        ...
        res.status(403).json({ message : "You are not an admin. You cannot update the Reviews"});
    }
}